<script lang="ts">
    ...
    import type { DropZone, ItemMetadata } from "../../typescript/types";
    import { DropZones } from "../../stores/drop-zones-store";

    export let draggableRef: HTMLElement, draggableItemMetadata: ItemMetadata;
        ...
        for(const zone of zones) {
            ...
            if(s1 && s2) {
                if(zone != hoveredDropZone) {
                    zone.hoverEnterCallback(draggableItemMetadata);
                }
                return zone;
            }
            if(hoveredDropZone) {
                hoveredDropZone.hoverLeaveCallback(draggableItemMetadata); 
            }
        }
        return null;
    }
